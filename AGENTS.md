...
| 沙箱 | `.grok/sandbox.toml`（`grok --sandbox grok-secure/grok-review`，内核级） |
| 门禁脚本 | `.grok/scripts/`：static-check、arch-check、codemap、ci-review（CI headless 门禁） |
| LSP | `.grok/lsp.json`（模板见 repo-navigator） |
| 内建 | `explore` / `plan` / `general-purpose`；勘察一律 explore |
| 记忆 | memory 开启时用 `/remember`、`/flush` 沉淀架构事实与决策 |
