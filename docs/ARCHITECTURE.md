...
| Sandbox | `.grok/sandbox.toml`（grok-secure / grok-review） |
| Arch rules | `.grok/arch/boundaries.txt` + `scripts/arch-check.*` |
| Gate scripts | `scripts/static-check.*`、`arch-check.*`、`codemap.*`、`ci-review.sh` |
...
- `PreToolUse` 可 deny（safe-shell / block-pkill / secrets-guard / no-direct-code-guard / pre-commit-check）
