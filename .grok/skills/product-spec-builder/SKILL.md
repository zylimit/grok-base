...
[需求维度清单]
    ...
    **非功能需求（五性，尽量收集）**：
    - 先问一句定档："这个系统挂了/被攻破，最坏伤害什么？（人身/设备/资金/数据/仅体验）"
    - 按档位追问关键项：外部依赖挂了怎么办（韧性）、谁能访问什么（安全）、
      破坏性操作要不要二次确认（功能安全）、收集哪些个人数据凭什么收（隐私）、
      多少人用/坏了能忍多久（可靠性）
    - 答案落 Spec 的「非功能需求」节；需要完整 NFR-Spec 时提示主 Agent 走 nfr-gatekeeper skill
    - 用户全部说"不在乎" → 记录降档理由，不硬塞
