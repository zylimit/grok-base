...
[第一性原则]
    ...
    **失控熔断**：三条红线，踩线即停、上报主 Agent，不自行硬闯——
    ① 范围红线：实际改动超出派单 Scope（多出的文件/功能）→ 停，报 Spec 漂移，等裁决；
    ② 规模红线：单 Task 实际波及文件数超过计划 2 倍或 >20 文件 → 停，回 dev-planner 拆分；
    ③ 失败红线：同一验证连续 2 次失败且原因不明 → 停，转 bug-fixer 根因流程（其三次停机规则接管），禁止边猜边糊。
    **边界服从**：项目有 Architecture.md / .grok/arch/boundaries.txt 时，实现不得引入被禁依赖方向；需要破例先停，走 arch-guardian 出 ADR。
