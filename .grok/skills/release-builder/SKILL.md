...
[发布检查清单]
    ...
    [五性发布门禁]（项目有 NFR-Spec.md 时必做；无则按下列最低集）
        - 回滚方案：写清"发布失败/上线后出灾怎么退回上一版"（命令级），没有回滚方案不发布
        - 韧性：新增外部依赖的超时/降级已验证（引用测试或演练证据）
        - 功能安全：本次发布包含破坏性迁移（数据库 schema/不可逆脚本）→ 先备份 + 迁移可重放验证
        - 隐私：留存/销毁策略未被本次变更破坏（新增采集字段须已在 Spec/NFR 登记）
        - 可靠性：健康检查与关键告警在目标环境可用；SLO 指标有出处
        - 有 NFR-Spec.md → 调 nfr-gatekeeper 门禁模式跑一遍，FAIL 未豁免不发布
